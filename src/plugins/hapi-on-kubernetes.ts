// Copied and adjusted from https://github.com/NextStepGuru/hapi-on-kubernetes/blob/master/lib/plugin.js

import type {Server} from '@hapi/hapi'

function register(server: Server, _config: object) {
  ...
  const shutdownSystem = async function (err: unknown, location: string) {
    if (process.env['IS_SHUTDOWN_REQUESTED'] === 'true') {
      server.log('debug', 'Shutdown already initiated, not triggering another one.')
      return
    }
    ...
    server.log('debug', {msg: 'Shutdown requested from:', location, err})
  ...
  server.ext('onPreStop', async function () {
  ...
  server.ext('onPreStart', async function () {
  ...
  server.ext('onPostStart', function () {
    isServerOnline = true
    if (config.showDebug) {
  ...
  try {
    process.on('SIGTERM', async () => {
      shutdownSystem('SIGTERM', 'primary')
    })
